...
public class PharmacyManagerialScreen extends JFrame
  implements ActionListener, ItemListener
{
  ...
  lblAdrCountry,lblAdrDistrict,lblAdrRegion,lblAdrCity,lblCompanyCode,lblChainSubName,lblAssortiment,lblPharmacyType,
  lblPromo,lblActiveness,lblMedRep,lblParameters,lblDateParameters;
  ...
  cmbBoxCompanyCode,cmbBoxRepType,cmbBoxParType,cmbBoxSubChains,cmbBoxAssortiment,cmbBoxPharmacyType,
  cmbBoxPromo,cmbBoxActiveness,cmbBoxMedRep,cmbBoxDatePar;  
    ...
    EventQueue.invokeLater(new Runnable()
    {
      public void run() {
        try {
          ...
          PharmacyManagerialScreen pharmManScreen = new PharmacyManagerialScreen(inBag);
        }
        catch (Exception e) {
          e.printStackTrace();
        }
  ...
  public PharmacyManagerialScreen(ESIBag inBag) throws SQLException{
    super("Pharmacy Managerial Screen");
    ...
    this.lblChainSubName = new JLabel("Chain Sub Name");
    this.lblMedRep = new JLabel("Medical Representative");
    
    this.lblAssortiment = new JLabel("Assortiment");
    this.lblPharmacyType = new JLabel("Pharmacy Type");   
    this.lblPromo = new JLabel("Promo");
    this.lblActiveness = new JLabel("Activeness");
    ...
    Util.getPRMData("group_company", "solgar_prm.prm_sales_chains",cmbBoxCompanies);
    this.cmbBoxCompanies.insertItemAt("", 0);
    this.cmbBoxCompanies.setMaximumRowCount(50);
    this.cmbBoxCompanies.setSelectedIndex(-1);
    
    this.cmbBoxSubChains = new JComboBox(new String[0]);
    Util.getPRMData("group_company", "solgar_prm.prm_sales_sub_chains",cmbBoxSubChains);
    this.cmbBoxSubChains.insertItemAt("", 0);
    this.cmbBoxSubChains.setMaximumRowCount(50);
    this.cmbBoxSubChains.setSelectedIndex(-1);
    ...
    this.cmbBoxParType.addItem("TOTAL_ACTIVENESS");
    this.cmbBoxParType.setSelectedIndex(0);

    
    this.cmbBoxAssortiment = new JComboBox(new String[0]);
    Util.getPRMDataGrpBy("assortiment", "solgar_prm.prm_sales_assortiment",cmbBoxAssortiment);
    this.cmbBoxAssortiment.insertItemAt("", 0);
    this.cmbBoxAssortiment.setMaximumRowCount(50);
    this.cmbBoxAssortiment.setSelectedIndex(-1);
    
    cmbBoxPharmacyType = new JComboBox( new String[]{});		
	Util.getPRMData("pharmacytype", "solgar_prm.prm_sales_pharmacytype",cmbBoxPharmacyType);
	cmbBoxPharmacyType.insertItemAt("", 0);
	cmbBoxPharmacyType.setMaximumRowCount(50);
	cmbBoxPharmacyType.setSelectedIndex(-1);
	
	cmbBoxPromo = new JComboBox( new String[]{});		
	Util.getPRMData("promo", "solgar_prm.prm_sales_promo",cmbBoxPromo);	
	cmbBoxPromo.insertItemAt("", 0);
	cmbBoxPromo.setMaximumRowCount(50);
	cmbBoxPromo.setSelectedIndex(-1);
	
	cmbBoxActiveness = new JComboBox( new String[]{});		
	...
	cmbBoxActiveness.setSelectedIndex(-1);
    ...
    this.paramPanel.add(this.lblChainSubName);
    this.paramPanel.add(this.cmbBoxSubChains);    
    this.paramPanel.add(this.lblAdrDistrict);
    this.paramPanel.add(this.cmbBoxDistrict);   
    this.paramPanel.add(this.lblAssortiment);
    this.paramPanel.add(this.cmbBoxAssortiment);   
    this.paramPanel.add(this.lblAdrRegion);
    this.paramPanel.add(this.cmbBoxRegion);    
    this.paramPanel.add(this.lblPharmacyType);
    this.paramPanel.add(this.cmbBoxPharmacyType);    
    ...
    this.paramPanel.add(this.lblPromo);
    this.paramPanel.add(this.cmbBoxPromo);   
    this.paramPanel.add(lblDateParameters);
    this.paramPanel.add(cmbBoxDatePar); 
    this.paramPanel.add(lblActiveness);
    this.paramPanel.add(cmbBoxActiveness);
  ...
  public void actionPerformed(ActionEvent e)
  {
    try {
      if (e.getActionCommand().equals("Generate Report"))
      {
        ...
        String selectedCountry= "",selectedDistrict = "",selectedRegion = "",selectedCity = "",selectedChain = "",
        		selectedBrand = "",selectedMedRep = "",selectedSubChains = "",selectedPrdMain = "",selectedPrdSub = "",repType = "",
        		selectedAssortiment = "",selectedPharmacyType="",selectedPromo="",selectedActiveness="",selectedFromPar="",selectedDatePar="";
        ...
        if ((this.cmbBoxSubChains.getSelectedItem() != null) && (this.cmbBoxSubChains.getSelectedItem().toString().trim().length() > 0)) {
        	selectedSubChains = this.cmbBoxSubChains.getSelectedItem().toString();
        }

        if ((this.cmbBoxAssortiment.getSelectedItem() != null) && (this.cmbBoxAssortiment.getSelectedItem().toString().trim().length() > 0)) {
        	selectedAssortiment = this.cmbBoxAssortiment.getSelectedItem().toString();
        }
        
        if ((this.cmbBoxPharmacyType.getSelectedItem() != null) && (this.cmbBoxPharmacyType.getSelectedItem().toString().trim().length() > 0)) {
        	selectedPharmacyType = this.cmbBoxPharmacyType.getSelectedItem().toString();
        }
        if ((this.cmbBoxPromo.getSelectedItem() != null) && (this.cmbBoxPromo.getSelectedItem().toString().trim().length() > 0)) {
        	selectedPromo= this.cmbBoxPromo.getSelectedItem().toString();
        }
        
        if ((this.cmbBoxActiveness.getSelectedItem() != null) && (this.cmbBoxActiveness.getSelectedItem().toString().trim().length() > 0)) {
        	selectedActiveness= this.cmbBoxActiveness.getSelectedItem().toString();
        }
        ...
        {
    	  DefaultTableModel localDefaultTableModel = ReportQueries.repPharmManagerial(
          this.resultTable, selectedBeginDate,selectedEndDate,selectedCountry,selectedDistrict,selectedRegion,selectedCity,
          selectedChain,selectedBrand,repType,selectedSubChains,selectedAssortiment,selectedPharmacyType,selectedPromo,selectedActiveness,selectedMedRep,
