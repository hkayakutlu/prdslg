...
public class ExpsAppScreen extends JFrame implements ActionListener,ItemListener,MouseListener,FocusListener{
	 ...
	 */
	public ExpsAppScreen(ESIBag inBag) {
			...
			if (e.getActionCommand().equals("Approve")) {
				for (int j = 0; j < model.getRowCount(); j++){
					if(resultTable.getValueAt(j, 0).toString().equalsIgnoreCase("true")){					
						ConnectToDb.statuChangeMarktExps(resultTable.getValueAt(j, 1).toString(), userName,"2");
					}
				}				
				...
				for (int j = 0; j < model.getRowCount(); j++){
					if(resultTable.getValueAt(j, 0).toString().equalsIgnoreCase("true")){					
						ConnectToDb.statuChangeMarktExps(resultTable.getValueAt(j, 1).toString(), userName,"3");
					}
				}
				for( int i = model.getRowCount() - 1; i >= 0; i-- ) {
					model.removeRow(i);
				...
				if (firstStage.equalsIgnoreCase("Мероприятия")) {
					...
					if (secondStage.equalsIgnoreCase("MK") || secondStage.equalsIgnoreCase("PAS & Master-Set")||
						...
						if (additionalExps.length() > 0) {
							...
							secondStage.equalsIgnoreCase("Мини-презентации")) {
						tempBag.put("TABLE", k, "PAYMENTTYPE", secondStage);
						tempBag.put("TABLE", k, "TOTALCOUNT", totalCount);
						tempBag.put("TABLE", k, "TOTALSUM", totalSum);
						k++;
					...
					}else if (secondStage.equalsIgnoreCase("Внутрисетевые обучения")) {
						tempBag.put("TABLE", k, "PAYMENTTYPE", secondStage);
						tempBag.put("TABLE", k, "TOTALCOUNT", totalCount);
						tempBag.put("TABLE", k, "TOTALSUM", totalSum);
						k++;
					...
					} else if (secondStage.equalsIgnoreCase("Дни Solgar")) {
						tempBag.put("TABLE", k, "PAYMENTTYPE", secondStage);
						tempBag.put("TABLE", k, "TOTALCOUNT", totalCount);
						tempBag.put("TABLE", k, "TOTALSUM", totalSum);
						k++;
					...
					}  else if (secondStage.equalsIgnoreCase("Расходы на мед.представителей")||
							secondStage.equalsIgnoreCase("Расходы на партнеров")) {
						tempBag.put("TABLE", k, "PAYMENTTYPE", secondStage);
						tempBag.put("TABLE", k, "TOTALCOUNT", totalCount);
						tempBag.put("TABLE", k, "TOTALSUM", totalSum);
						k++;
					...
					}   else if (secondStage.equalsIgnoreCase("Централизованные обучения")) {
						tempBag.put("TABLE", k, "PAYMENTTYPE", secondStage);
						tempBag.put("TABLE", k, "TOTALCOUNT", totalCount);
						tempBag.put("TABLE", k, "TOTALSUM", totalSum);
						k++;
