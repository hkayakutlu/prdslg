...
import java.awt.event.MouseEvent;
	...
	public class ChainExpsCampaignEnrtyScreen extends JFrame implements ActionListener,ItemListener,MouseListener{
		...
		private JTable tblTarget,tblRelease,tableResult,tblSearch,tblDiscountSales;
		...
		final DefaultTableModel modelDiscount = new DefaultTableModel();
		...
		String headerDiscount[] = new String[] {"Id", "Year", "Month", "Discount Rate", "Realization", "Pack", "Sum"};
			...
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					try {
						...
						ChainExpsCampaignEnrtyScreen frame = new ChainExpsCampaignEnrtyScreen(inBag);
		 ...
		 * @throws SQLException 
		 */
		public ChainExpsCampaignEnrtyScreen(ESIBag inBag) throws SQLException {
			super("Chain Campaign Payments Entry");
			Toolkit toolkit;
			Dimension dim;
			...
			pnlStage.setBounds(601, 277, 551, 201);
			...
			cmbBoxAdvertisementType.setBounds(410, 82, 123, 22);
			...
			cmbBoxDocumentStatus.setBounds(410, 111, 123, 18);
			...
			pnlAgreement.setBounds(243, 110, 261, 156);
			...
			agreementEndDate.setBounds(148, 126, 108, 20);
			...
			pnlTargets.setBounds(504, 110, 378, 156);
			...
			scrollTarget.setBounds(10, 29, 358, 116);
			...
			pnlRelease.setBounds(892, 110, 261, 156);
			...
			scrollRelease.setBounds(10, 28, 241, 117);
			...
			tblRelease.setModel(new DefaultTableModel(
				...
				new String[] {
					"Id","Year", "Month", "Box", "Amount"
				}
			) {
				Class[] columnTypes = new Class[] {
						Integer.class,String.class, Object.class, Integer.class, Double.class
				};
				public Class getColumnClass(int columnIndex) {
					return columnTypes[columnIndex];
				}
			});
			tblRelease.getColumnModel().getColumn(0).setPreferredWidth(10);
			tblRelease.getColumnModel().getColumn(1).setPreferredWidth(40);
			tblRelease.getColumnModel().getColumn(2).setPreferredWidth(44);
			tblRelease.getColumnModel().getColumn(3).setPreferredWidth(33);
			tblRelease.getColumnModel().getColumn(4).setPreferredWidth(59);
			tblRelease.getColumnModel().getColumn(1).setCellEditor(new DefaultCellEditor(cmbYear));
			tblRelease.getColumnModel().getColumn(2).setCellEditor(new DefaultCellEditor(cmbMonth));
			...
			pnlResult.setBounds(10, 489, 1133, 146);
			...
			scrollResult.setBounds(10, 33, 1113, 105);
			...
			pnlSearch.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Agreement", TitledBorder.LEADING, TitledBorder.ABOVE_TOP, null, new Color(0, 0, 0)));
			...
			JPanel pnlDiscountSales = new JPanel();
			pnlDiscountSales.setBorder(new TitledBorder(null, "Discount Sales", TitledBorder.LEADING, TitledBorder.ABOVE_TOP, null, null));
			pnlDiscountSales.setBounds(20, 277, 551, 201);
			getContentPane().add(pnlDiscountSales);
			pnlDiscountSales.setLayout(null);
			
			JScrollPane scrollDiscountSales = new JScrollPane();
			scrollDiscountSales.setBounds(10, 29, 531, 161);
			pnlDiscountSales.add(scrollDiscountSales);
			
			tblDiscountSales = new JTable();
			tblDiscountSales.setModel(new DefaultTableModel(
				new Object[][] {
				...
				new String[] {
					"Id", "Year", "Month", "Discount Rate", "Realization", "Pack", "Sum"
				}
			) {
				Class[] columnTypes = new Class[] {
			...
			tblDiscountSales.getColumnModel().getColumn(0).setPreferredWidth(32);
			tblDiscountSales.getColumnModel().getColumn(1).setPreferredWidth(47);
			tblDiscountSales.getColumnModel().getColumn(2).setPreferredWidth(42);
			tblDiscountSales.getColumnModel().getColumn(3).setPreferredWidth(80);
			tblDiscountSales.getColumnModel().getColumn(6).setPreferredWidth(96);
			tblDiscountSales.getColumnModel().getColumn(1).setCellEditor(new DefaultCellEditor(cmbYear));
			tblDiscountSales.getColumnModel().getColumn(2).setCellEditor(new DefaultCellEditor(cmbMonth));
			scrollDiscountSales.setViewportView(tblDiscountSales);
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				if (e.getActionCommand().equals("ADD")) {
					...
					int rowBonus = tblDiscountSales.getRowCount();
					
					//AgreementBase Info
					outBag.put("EXPENSETYPE", "CAMPAIGN");
					...
					outBag.put("AREA", cmbBoxArea_1.getSelectedItem().toString());
					outBag.put("REGION", cmbBoxRegion_1.getSelectedItem().toString());
					outBag.put("CITY", cmbBoxCity_1.getSelectedItem().toString());
					...
					for (int j = 0; j  < rowRelease; j++) {
						...
						formatToBagValueRelease(outBag, j, 2, "RELEASETABLE", "BOXCOUNT",true);
						formatToBagValueRelease(outBag, j, 3, "RELEASETABLE", "AMOUNT",true);					
					}
					//Bonus
		...
		private void formatToBagValueBonus(ESIBag outBag,int rowNum,int index,String tableBagkey,String valueBagKey,boolean number){			
			if(tblDiscountSales.getValueAt(rowNum, index) != null && tblDiscountSales.getValueAt(rowNum, index).toString().length()>0){
				outBag.put(tableBagkey,rowNum,valueBagKey, tblDiscountSales.getValueAt(rowNum, index).toString());
			}else{
				if(number){
					outBag.put(tableBagkey,rowNum,valueBagKey, "0");
				}else{
					...
					modelRelease.addRow(new Object [] 
			        		{
			        			...
			        			tempBag.get("TABLERELEASE",j,"BOX"),
		...
		private void createTabelDiscountModel(ESIBag tempBag) {
		    try{
				modelDiscount.setColumnIdentifiers(headerDiscount);
				tblDiscountSales.setModel(modelDiscount);
		    	
				for (int j = 0; j < tempBag.getSize("TABLEDISCOUNT"); j++){
					modelDiscount.addRow(new Object [] 
			        		{
			        			tempBag.get("TABLEDISCOUNT",j,"ID"),
			        			tempBag.get("TABLEDISCOUNT",j,"YEAR"),
			        			tempBag.get("TABLEDISCOUNT",j,"MONTH"),
			        			tempBag.get("TABLEDISCOUNT",j,"DISCOUNTRATE"),
			        			tempBag.get("TABLEDISCOUNT",j,"REALIZATION"),
			        			tempBag.get("TABLEDISCOUNT",j,"PACK"),
			        			tempBag.get("TABLEDISCOUNT",j,"SUM")
			        		});		
				}
				tblDiscountSales.getColumnModel().getColumn(1).setCellEditor(new DefaultCellEditor(cmbYear));
				tblDiscountSales.getColumnModel().getColumn(2).setCellEditor(new DefaultCellEditor(cmbMonth));
		...
		public void mouseClicked(MouseEvent e) {
			try {	
				if (e.getComponent().getName().equals("SearchTabel")) {	
					...
					tempBag.put("EXPENSETYPE", "CAMPAIGN");
					...
					createTabelDiscountModel(tempBag);
