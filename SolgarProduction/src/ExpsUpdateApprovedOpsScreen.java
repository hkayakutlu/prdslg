...
public class ExpsUpdateApprovedOpsScreen extends JFrame implements ActionListener,ItemListener,MouseListener,FocusListener{
	...
	lblClinicName,lblContracter,lblContracter1,lblKeyLeader,lblKeyLeader1,lblAmount7,lblStatus;
	...
	cmbBoxExpRekChain,cmbBoxExpRekProduct,cmbBoxExpPosProduct,cmbBoxClinics,cmbBoxContracter,cmbBoxContracter1,cmbBoxKeyLeader,cmbBoxKeyLeader1,cmbBoxTema,
	cmbBoxEmpty,cmbBoxStatus;
	private JDateChooser startDate,endDate;
	private JTextField txtExpMerUcastnik,
	txtExpRekUsloviya,txtExpPosParyadk,txtExpPosStatus,txtComment,txtId,txtEmpty,txtEmpty1;
	public JButton btnAdd,btnSearch,btnUpdate,btnSave,btnExit,btnDelete;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					ExpsUpdateApprovedOpsScreen window = new ExpsUpdateApprovedOpsScreen(inBag);
	 ...
	 * @throws SQLException 
	 */
	public ExpsUpdateApprovedOpsScreen(ESIBag inBag) throws SQLException {
		super("Expense Update&Delete Approved Operations");
		Toolkit toolkit;
		Dimension dim;
		...
		cmbBoxEmpty = new JComboBox( new String[]{});				
		cmbBoxEmpty.setEditable(true);
		cmbBoxEmpty.setEnabled(false);
		...
		paramPanelExpTypes.add(lblStatus);
		paramPanelExpTypes.add(cmbBoxStatus);
		...
		try {
			if (e.getActionCommand().equals("Add")) {
				...
				try{
					...
					Dispatcher.updateMarktExps(resultTable, userName, i,2);
				}catch (Exception ef) {
				...
				boolean withoutError =true;
				try{
					Dispatcher.statuChangeMarktExps(resultTable.getValueAt(i, 0).toString(), userName,"3");
				}catch (Exception ef) {
					JOptionPane.showMessageDialog(pnlInfoMsg, ef.getMessage(), "Error in Delete", JOptionPane.ERROR_MESSAGE);
					withoutError = false;
				}
				...
				if(withoutError){
					...
					tempBag = Dispatcher.getMarktExpsWithParam(cmbBoxCountry,cmbBoxArea,cmbBoxCompanyCode,cmbBoxExpMain,cmbBoxExpLevel1,cmbBoxExpLevel2,
							cmbBoxEmpty,cmbBoxEmpty,cmbBoxEmpty,cmbBoxEmpty,cmbBoxStatus,null,null,null);
					createModel(tempBag);					
					JOptionPane.showMessageDialog(pnlInfoMsg, "Selected expense rejected", "Information", JOptionPane.INFORMATION_MESSAGE);
				}
	...
	public void itemStateChanged(ItemEvent itemEvent) {
	  ...
	  public void mouseClicked(MouseEvent e) {
			...
			SimpleDateFormat dcn1 = new SimpleDateFormat("yyyy-MM-dd");
		  	...
		  	txtId.setText(model.getValueAt(i, 0).toString());	//id	  			  	
