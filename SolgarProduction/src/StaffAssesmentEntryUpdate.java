...
import javax.swing.ScrollPaneConstants;
	...
	public class StaffAssesmentEntryUpdate extends JFrame implements ActionListener,ItemListener,MouseListener{
			...
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					try {
						...
						StaffAssesmentEntryUpdate frame = new StaffAssesmentEntryUpdate(inBag);
		 ...
		 * @throws SQLException 
		 */
		public StaffAssesmentEntryUpdate(ESIBag inBag) throws SQLException {
			super("Assesment Entry&Update");
			Toolkit toolkit;
			Dimension dim;
		...
		public void actionPerformed(ActionEvent e) {
			try {
				if (e.getActionCommand().equals("ADD")) {
					...
					setValuetoBafForSave(outBag);
					int i = tblResult.getSelectedRow();		
					outBag.put("RELATIONID", tblResult.getValueAt(i, 0).toString());
					boolean result = Dispatcher.updateAssesments(outBag);
					if(result){
					...
					ESIBag outBag = new ESIBag();
					setValuetoBafForSave(outBag);					
					boolean result = Dispatcher.saveAssesments(outBag);
					if(result){
						JOptionPane.showMessageDialog(pnlInfoMsg, "Operation completed succesfully", "Information", JOptionPane.INFORMATION_MESSAGE);
						cleanUp();
						for( int j = modelResult.getRowCount() - 1; j >= 0; j-- ) {
		...
		private void setValuetoBafForSave(ESIBag outBag) {
			int rowPharmacy = tblPharmacyNum.getRowCount();
			int rowDoctor = tblDocNum.getRowCount();
			int rowRelease = tblRealization.getRowCount();
			int rowCalculation = tblCalculation.getRowCount();
			
			//Assesment Base Info
			outBag.put("BRAND", cmbBoxBrand.getSelectedItem().toString());
			outBag.put("EMPLOYEEID", cmbBoxId.getSelectedItem().toString());
			...
			outBag.put("STAFFNAME", cmbBoxStaffName.getSelectedItem().toString());
			outBag.put("YEAR", cmbBoxYear.getSelectedItem().toString());
			outBag.put("MONTH", cmbBoxMonth.getSelectedItem().toString());
			outBag.put("MONTHLYVISIT", txtMonVisit.getText());
			outBag.put("WORKINGDAY", txtWorkingDay.getText());
			outBag.put("USERNAME", userName);
			
			//Pharmacy
			for (int j = 0; j  < rowPharmacy; j++) {					
				outBag.put("PHARMTABLE",j,"ORDER", String.valueOf(j));		
				formatToBagValue(outBag, j, 1, "PHARMTABLE", "PHARMACYTYPE",tblPharmacyNum);
				formatToBagValue(outBag, j, 2, "PHARMTABLE", "SUMTOTAL",tblPharmacyNum);
			}
			
			//Doctor
			for (int j = 0; j  < rowDoctor; j++) {					
				outBag.put("DOCTORTABLE",j,"ORDER", String.valueOf(j));		
				formatToBagValue(outBag, j, 1, "DOCTORTABLE", "DOCTORTYPE",tblDocNum);
				formatToBagValue(outBag, j, 2, "DOCTORTABLE", "SUMTOTAL",tblDocNum);
			}
			
			//Realization
			for (int j = 0; j  < rowRelease; j++) {					
				outBag.put("RELEASETABLE",j,"ORDER", String.valueOf(j));		
				formatToBagValue(outBag, j, 1, "RELEASETABLE", "BONUSTYPE",tblRealization);
				formatToBagValue(outBag, j, 2, "RELEASETABLE", "PLANS",tblRealization);
				formatToBagValue(outBag, j, 3, "RELEASETABLE", "REALIZATION",tblRealization);
				formatToBagValue(outBag, j, 4, "RELEASETABLE", "SUMTOTAL",tblRealization);
			}
			//Calculation
			for (int j = 0; j  < rowCalculation; j++) {					
				outBag.put("CALCTABLE",j,"ORDER", String.valueOf(j));		
				formatToBagValue(outBag, j, 1, "CALCTABLE", "BONUSTYPE",tblCalculation);
				formatToBagValue(outBag, j, 2, "CALCTABLE", "SUMTOTAL",tblCalculation);
			}
		}
		...
		private void formatToBagValue(ESIBag outBag,int rowNum,int index,String tableBagkey,String valueBagKey,JTable tblStat ){			
			if(tblStat.getValueAt(rowNum, index) != null && tblStat.getValueAt(rowNum, index).toString().length()>0){
				outBag.put(tableBagkey,rowNum,valueBagKey, tblStat.getValueAt(rowNum, index).toString());
			}else{
